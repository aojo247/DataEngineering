...
def lambda_handler(event, context):
    ...
    s3_object_key = 'NBA/raw/nba_all_star_list.csv'  # Set the desired object key for the raw file (remember- you still need to run a cleaning script)
    ...
    nba_all_star_df = pd.concat(dataframes, ignore_index=True)

 

    # Convert the DataFrame to CSV format and send it to S3
    csv_buffer = StringIO()
    nba_all_star_df.to_csv(csv_buffer, index=False)
